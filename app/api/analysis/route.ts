...
async function analyzeClusters() {
  ...
  if (embeddings.length < 3) {
    return { clusters: [], message: 'Need at least 3 responses for clustering' }
  }
...
async function analyzeFeatureImportance() {
  ...
  const featureScores: Record<string, { sum: number; count: number; avg: number }> = {
  ...
  Object.keys(featureScores).forEach(feature => {
    if (featureScores[feature].count > 0) {
      featureScores[feature].avg = featureScores[feature].sum / featureScores[feature].count
    }
  })
...
async function analyzeCulturalPatterns() {
  ...
  const culturalMotivation: Record<string, number> = { Yes: 0, No: 0, Maybe: 0 }

  surveys.forEach(survey => {
    if (survey.section_a?.country) {
      countries[survey.section_a.country] = (countries[survey.section_a.country] || 0) + 1
    }
...
async function analyzeSentiment() {
  ...
  [...surveys, ...interviews].forEach(response => {
...
function analyzeClusterCharacteristics(clusterSurveys: any[]) {
  ...
  const avgActivity = clusterSurveys
    .map(s => {
  ...
  const avgEngagement = clusterSurveys
    .map(s => {
  ...
  const culturalInterest = clusterSurveys.filter(
    s => s.section_c?.culturalMotivation === 'Yes'
